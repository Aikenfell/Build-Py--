...
def CArray():
    ...
    inte.update({"cell2int" : cell2int })
    inte.update({"int2cell" : int2cell })
    vv = {"IDs" : {} , "Head" : [] ,"Chest" : [] ,"Legs" : [] ,"Hands" : [] ,"Weapon" : [] ,"Cells" : {}}
    for item in gear2int:
        if item != "None":
            vv["IDs"].update({ gear2int[item]["ID"] : {1 : gear2int[item]["Innate Cell"] , 2 : gear2int[item]["Cell Slot One Type"] , 3 : gear2int[item]["Cell Slot Two Type"]}})
            if gear2int[item]["Region"] == 1:
                vv["Head"].append(gear2int[item]["ID"])
            elif gear2int[item]["Region"] == 2:
                vv["Chest"].append(gear2int[item]["ID"])
            elif gear2int[item]["Region"] == 3:
                vv["Legs"].append(gear2int[item]["ID"])
            elif gear2int[item]["Region"] == 4:
                vv["Hands"].append(gear2int[item]["ID"])
            else:
                vv["Weapon"].append(gear2int[item]["ID"])
    for item in cell2int:
        if item != "None":
            vv["Cells"].update({cell2int[item][0] : cell2int[item][1]})
        
                
    with open("Head.json", "w") as filez:
        json.dump(vv["Head"], filez, indent=4)
    with open("Chest.json", "w") as filez:
        json.dump(vv["Chest"], filez, indent=4)
    with open("Hands.json", "w") as filez:
        json.dump(vv["Hands"], filez, indent=4)
    with open("IDs.json", "w") as filez:
        json.dump(vv["IDs"], filez, indent=4)
    with open("Legs.json", "w") as filez:
        json.dump(vv["Legs"], filez, indent=4)
    with open("CARR.json", "w") as filez:
        json.dump(vv, filez, indent=4)
